          ...
          if (savedComboInstances) {
            try {
              ...
              if (parsed.length >= 10) {
                ...
                const parsedKeys = new Set<string>(parsed.map((c: Omit<ComboInstance, 'dateModified'> & { dateModified?: string }) => `${c.catalogId}-${c.displayName}`));
                const prevKeys = new Set<string>(prev.map(ci => `${ci.catalogId}-${ci.displayName}`));
                // If they're the same, return prev to avoid unnecessary updates
                if (parsedKeys.size === prevKeys.size && Array.from(parsedKeys).every(k => prevKeys.has(k))) {
                  return prev;
                }
